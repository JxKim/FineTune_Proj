...
def create_answer_mask(input_ids,tokenizer:PreTrainedTokenizerFast):
    """
    构建answer mask，用于后续训练过程中，只关注answer部分的loss
    Args:
...
def compute_loss(output_logits,target_labels,assistant_answer_mask):
    ...
    # 获取到当前batch当中，每个样本的有效token数，用以后续计算平均loss
    tokens_per_sample = assistant_answer_mask.sum(dim=-1)

    # 计算每个样本的平均loss
    sample_losses  = masked_token_losses.sum(-1) / tokens_per_sample

    # 计算当前batch的平均loss
    step_average_loss = torch.nanmean(sample_losses)
...
def train(model,config,tokenizer):
    ...
    for step in range(total_steps):
        ...
        logger.info(f"当前step为{step}, tokens_per_sample为{tokens_per_sample}")
        min_answer_tokens = tokens_per_sample.min().item()
        if min_answer_tokens == 0:
            logger.info(f"当前step {step+1}中，无有效answer token，跳过该批次")
        ...
        # 4、计算当前batch的平均loss
        batch_average_loss = compute_loss(output_logits=model_logits,target_labels=target_labels,assistant_answer_mask=final_loss_mask)
        ...
        # 6、判断当前step是否需要达到了梯度累计的steps数，如果是，则需要更新参数
        is_accumulation_step = (step + 1) % config.gradient_accumulation_steps == 0
        is_final_step = (step + 1) == total_steps
        if is_accumulation_step or is_final_step:
            optimizer.step()
            optimizer.zero_grad()
        
        actual_batch_loss = batch_average_loss.item() * config.gradient_accumulation_steps
        training_losses.append(actual_batch_loss)

        should_log = (step+1) % config.log_iter == 0 or is_final_step
        if should_log:
            recent_losses = training_losses[-config.log_iter:]
    ...
    logger.info("训练完成")
    logger.info("训练统计：")
    logger.info(f"          -总批次数：{total_steps}")
    logger.info(f"          -跳过批次数：{skipped_batches_count}")
    logger.info(f"          -有效批次数：{total_steps - skipped_batches_count}")
    logger.info(f"          -最终平均损失：{np.nanmean(training_losses[-100:]):.4f}")
...
def save_model_tokenizer(model,tokenizer,save_path):
    ...
    logger.info(f"模型和分词器已保存至{save_path}")

def logging_setting(level):
    import logging
    logging_level = level.upper()
    if logging_level not in ["DEBUG","INFO"]:
        logging_level = "INFO"
...
def model_generate(model_path,prompt):
    ...
    original_input_ids = tokenizer.apply_chat_template(messages,tokenize=True,add_generation_prompt=True,truncation=True,max_length=2500,return_tensors="pt")


    input_ids = original_input_ids["input_ids"].to("cuda")
    attention_mask = original_input_ids["attention_mask"].to("cuda")
    res = model.generate(input_ids = input_ids,attention_mask = attention_mask,max_new_tokens = 500)


    decode_res = tokenizer.decode(res,skip_special_tokens = True)
    print(decode_res[0])
...
def main():
    ...
    train_parser.add_argument("--logging-level",type=str,default="info",help="日志级别，可选值为debug,info")
    train_parser.add_argument("--train_data_size",type=int,default=10,help="训练数据集的大小")
    train_parser.add_argument("--batch_size",type=int,default=2,help="批次大小")
    train_parser.add_argument("--output_dir",type=str,default="./finetuned/Qwen3-0.6B-SFT",help="SFT模型输出目录")
    generate_parser = subparsers.add_parser("generate")
    generate_parser.add_argument("--prompt",type=str,default="",help="要生成的提示")
    generate_parser.add_argument("--model_path",type=str,default="./finetuned/Qwen3-0.6B-SFT",help="模型路径")
    ...
    if args.command == "train":
        ...
        logging_setting(logging_level)
        
        # 2、创建配置实例
        config = SFTConfig(train_data_size=args.train_data_size)
        ...
        model_generate(model_path=args.model_path,prompt=args.prompt)
